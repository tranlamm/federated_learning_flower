...
sys.path.append('/home/bkcs/Lam_Quang/dql')
sys.path.append('/home/bkcs/Lam_Quang/ucb1')
...
from dql_epsilon_agent import Agent
import time
...
# Variable
ucb1 = UCB1(action_dims_ucb1)
agent = Agent(state_size=state_dims, action_size=action_dims_dql, seed=0)
train_time = []
val_loss = []
val_acc = []
reward = []
...
class CustomStrategy(fl.server.strategy.FedAvg):
    ...
    def aggregate_fit(
        ...
        if self.pre_action_ucb1 is not None:
            ucb1.update(self.pre_action_ucb1, curr_reward)
        
        # add to experiment buffer
        if self.pre_state is not None:
            agent.step(self.pre_state, self.pre_action_dql, curr_reward, curr_state)
        ...
        if (action == 0):
            self.pre_action_dql = random.choice(np.arange(action_dims_dql))
        else:
            self.pre_action_dql = agent.act(curr_state)
            
        if self.pre_action_dql == 0:
            timeDiff = -1
        elif self.pre_action_dql == 1:
            timeDiff = 0
        else:
            timeDiff = 1
