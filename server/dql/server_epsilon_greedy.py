...
# Variable
agent = Agent(state_size=state_dims, action_size=action_dims, seed=0)
train_time = []
val_loss = []
val_acc = []
reward = []
...
class CustomStrategy(fl.server.strategy.FedAvg):
    ...
    def aggregate_fit(
        ...
        if self.pre_state is not None:
            agent.step(self.pre_state, self.pre_action, curr_reward, curr_state)
        self.pre_state = curr_state
        
        # predict next action
        action = agent.act(curr_state, self.epsilon)
