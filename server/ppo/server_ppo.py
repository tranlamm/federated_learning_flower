...
# Variable
agent = PPOAgent(state_dims=state_dims, hidden_dims=64, action_dims=action_dims)
train_time = []
val_loss = []
val_acc = []
reward = []
...
class CustomStrategy(fl.server.strategy.FedAvg):
    ...
    def aggregate_fit(
        ...
        if self.pre_state is not None:
            agent.memory.push(self.pre_state, self.pre_action, self.pre_log_prob, curr_reward, curr_state)
        self.pre_state = curr_state
        
        # predict next action
        action, log_prob, _ = agent.choose_action(curr_state)
