...
sys.path.append('/home/bkcs/Lam_Quang/ddpg')
...
agent = DDPGagent(state_dims=state_dims, action_dims=action_dims_ddpg)
train_time = []
val_loss = []
val_acc = []
reward = []
...
class CustomStrategy(fl.server.strategy.FedAvg):
    ...
    def aggregate_fit(
        ...
        if self.pre_state is not None:
            agent.memory.push(self.pre_state, self.pre_action_ddpg, curr_reward, curr_state)
        ...
        if (action == 0):
            self.pre_action_ddpg = random.uniform(-1, 1)
        else:
            self.pre_action_ddpg = agent.get_action(curr_state)
        
        nextTrainTime = self.train_time + self.pre_action_ddpg
        if nextTrainTime > MAX_TIME:
            nextTrainTime = MAX_TIME
        elif nextTrainTime < MIN_TIME:
            nextTrainTime = MIN_TIME
                
        self.pre_action_ddpg = np.array([self.pre_action_ddpg])
