...
class SACAgent:
    ...
    def __init__(self, state_dims, action_dims, hidden_dims, batch_size):
        ...
        self.critic_local = Network(input_dimension=self.state_dims,
                                    output_dimension=self.action_dims,
                                    hidden_dimension=hidden_dims)
        self.critic_local2 = Network(input_dimension=self.state_dims,
                                     output_dimension=self.action_dims,
                                     hidden_dimension=hidden_dims)
        ...
        self.critic_target = Network(input_dimension=self.state_dims,
                                     output_dimension=self.action_dims,
                                     hidden_dimension=hidden_dims)
        self.critic_target2 = Network(input_dimension=self.state_dims,
                                      output_dimension=self.action_dims,
                                      hidden_dimension=hidden_dims)
    ...
    def get_action_nondeterministically(self, state):
        ...
        discrete_action = np.random.choice(range(self.action_dims), p=action_probabilities)
        return discrete_action
    ...
    def train_networks(self, state, discrete_action, reward, next_state):
        ...
        if len(self.replay_buffer) >= self.REPLAY_BUFFER_BATCH_SIZE:
            ...
            states, actions, rewards, next_states = self.replay_buffer.sample(self.REPLAY_BUFFER_BATCH_SIZE)
            ...
            critic_loss, critic2_loss = \
                self.critic_loss(states, actions, rewards, next_states)
    ...
    def critic_loss(self, states_tensor, actions_tensor, rewards_tensor, next_states_tensor):
        ...
        soft_q_values = self.critic_local(states_tensor).gather(1, actions_tensor.long().unsqueeze(-1)).squeeze(-1)
        soft_q_values2 = self.critic_local2(states_tensor).gather(1, actions_tensor.long().unsqueeze(-1)).squeeze(-1)
